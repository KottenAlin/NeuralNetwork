...
for epoch in range(epochs):
    ...
    if epoch % 10 == 0:
        model.eval()
        x_val, y_val = get_batches(val_data, sequence_size)
        val_outputs = model(x_val)
        val_loss = criterion(val_outputs.reshape(-1, vocab_size), y_val.reshape(-1))
...
fc_params = model.fc_out.weight.numel() + model.fc_out.bias.numel()

print(f'\nParameter breakdown:')
print(f'Embedding layer: {embed_params:,}')
